...
downstream_module = load_source_module("downstream_module", "05_train_multitask_nids.py")
...
DEFAULT_CHECKPOINT = str(EXPERIMENT_DIR / "checkpoints" / "nids_multitask_05_v2" / "nids_multitask_best.pt")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run v2 downstream NIDS inference and print human-readable alerts."
    )
    parser.add_argument("--checkpoint", default=DEFAULT_CHECKPOINT, help="Path to the v2 downstream checkpoint.")
    parser.add_argument(
        "--split",
        default="test",
        help="Dataset split to score, for example validation, test, or test_ood.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override the future warning horizon stored in the checkpoint.",
    )
